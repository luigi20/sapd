...
function getValor(valor_campo){
	var substituir = document.getElementById(valor_campo).value.replace('.','');
	var valor = substituir.replace(',','.');
	return parseFloat(valor) * 100;
}

function calcularEvento() {
  var total = getValor('income') - getValor('cost');
  id('profit').value = (total / 100).toFixed(2).replace('.',',') ;
}

function calcularIngresso() {
  var total = document.getElementById('ticketValue').value * document.getElementById('ticketQtd').value;
  id('ticketProfit').value = (total).toFixed(2).replace('.',',') ;
}
...
function filtraNumerosComPonto(campo) {
    var s = "";
    var cp = "";
    vr = campo;
    tam = vr.length;
...
function formataDouble(campo, evt) {
    ...
    campo.value = filtraNumerosComPonto(campo.value);
    MovimentaCursor(campo, xPos);
}
